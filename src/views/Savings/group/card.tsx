      ...
      >
        ...
        <div className="p-3">
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div>
              ...
              <p className="font-semibold">
                ${group.period_amount * group.minimum_members}
              </p>
            </div>
            <div className="text-right">
              <p className="text-sm text-gray-500">Created Date</p>
              <p className="font-semibold">
                {getDateHumanReadable(group.created_at)}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <div className="text-right">
              <p className="text-sm text-gray-500">Payout Interval</p>
              <p className="font-semibold">{group.duration} days</p>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="flex items-center">
                <div className="flex -space-x-2">
                  {group.member_ids.slice(0, 3).map((id, idx) => (
                    <Avatar size={24} number={id} key={idx} />
                  ))}
                </div>
                {group.member_ids.length > 3 && (
                  <p className="text-sm ml-2">
                    +{group.member_ids.length - 3} members
                  </p>
                )}
              </div>
            </div>
            <div>
              ...
              <p className="font-semibold">
                {group.minimum_members - group.member_ids.length} members
              </p>
            </div>
          </div>
        </div>
